...
export default function Research() {
    return (
        <div className="w-full rounded-lg bg-white px-2 py-12 shadow-md dark:bg-gray-600 sm:px-12">
            <h1 className="mb-8 text-center text-3xl font-bold">{'Teaching Assistant Experience'}</h1>
                    ...
                    <tbody className="">
                        {taData.map((ta, idx) => (
                            <MyTableRow key={idx} idx={idx}>
                                <th className="flex basis-1/4 items-center max-sm:basis-1/3">{ta[0]}</th>
                                <td className="flex basis-3/4 items-center px-5 max-sm:basis-2/3">{ta[1]}</td>
                            </MyTableRow>
                        ))}
                    </tbody>
                </table>
