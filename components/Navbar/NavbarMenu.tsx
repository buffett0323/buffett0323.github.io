...
// react new class
export const NavbarMenu = ({ setOpenMenu, items, theme, setTheme, mounted }: NavbarMenuProps) => {
  const router = useRouter();

  return (
    <div
      className="fixed left-0 right-0 top-0 z-nav-1 h-screen w-full bg-black bg-opacity-50"
      onClick={() => setOpenMenu(false)}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="z-nav-2 space-y-2 rounded-lg bg-white p-5 shadow-md dark:bg-gray-800"
      >
        {items.map((item, index) => {
          return (
            <Link
              href={item[1]}
              className={`rounded-lg border border-transparent px-3 py-2 text-base hover:bg-[#F1F1F1] hover:shadow-lg hover:dark:bg-gray-700 ${
                router.asPath === item[1]
                  ? 'block font-bold text-gray-800 dark:text-gray-200'
                  : 'block font-normal text-gray-600 dark:text-gray-400'
              }`}
              key={index}
              onClick={() => setOpenMenu(false)}
            >
        ...
        {/* separate line */}
        <div className="mb-12 mt-5 box-border border border-gray-400 dark:border-gray-400 md:my-2"></div>
        <div className="flex flex-row items-center justify-center pt-5">
          ...
          <button
            ...
            type="button"
            className="ml-4 rounded-lg p-2 hover:bg-[#F1F1F1] hover:shadow-lg focus:outline-none hover:dark:bg-gray-700"
